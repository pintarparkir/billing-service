...
| `SERVICE_URL_DEV` | optional | — | Public URL of the dev Cloud Run revision (e.g. `https://billing-service-dev-abc.a.run.app`). When set, the smoke job probes `/healthz`. |
...
REPO_NAME=billing-service        # Repository name
SA_NAME=billing-deployer
...
```bash
gcloud run deploy billing-service-dev \
...
gcloud run services describe billing-service-prod --region=$REGION \
  --format='value(status.traffic)'

# Point all traffic back at it
gcloud run services update-traffic billing-service-prod \
  --region=$REGION \
